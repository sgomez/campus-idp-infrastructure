...
def store_images(dict_yaml, dest):
   ...
   for lang in dict_yaml['idp']['md']:
    ...
    if (lang == 'en'):
       ...
       flag = requests.get("http://flagpedia.net/data/flags/w580/%s.png" % lang)

    idp_images_dir = "%s/%s/idp/styles/%s" % (dest,dict_yaml['fqdn'],lang)
    pla_images_dir = "%s/%s/phpldapadmin/images" % (dest,dict_yaml['fqdn'])
...
def get_sso_credentials(dict_yaml, idp_credentials_dir='tmp'):
   for cert in dict_yaml['idp']['sso_keys']:
    if (cert['use'] == 'sign'):
     ...
     idp_sign_crt_file = open(idp_credentials_dir+"/idp-signing.crt","w")
     idp_sign_crt_file.write(idp_signing_crt)
     idp_sign_crt_file.close()

     # Create IDP Signing KEY file
     idp_sign_key_file = open(idp_credentials_dir+"/idp-signing.key","w")
    ...
    if (cert['use'] == 'enc'):
     ...
     idp_enc_crt_file = open(idp_credentials_dir+"/idp-encryption.crt","w")
     idp_enc_crt_file.write(idp_encryption_crt)
     idp_enc_crt_file.close()

     # Create IDP Encryption KEY file
     idp_enc_key_file = open(idp_credentials_dir+"/idp-encryption.key","w")
...
def get_sealer_keystore_pw(dict_yaml, ans_vault_file=None, dest='tmp', debug=False):
   ...
   idp_cred_pw = check_output(shlex.split("openssl rand -base64 27")).strip().decode("utf-8")
   ...
   idp_bin_dir = check_output(shlex.split('find / -path "*shibboleth-identity-provider-*/bin"')).strip().decode("utf-8")
   ...
   sealer_jks = check_output(shlex.split('find '+credentials_dir+' -name "sealer.jks"')).strip().decode("utf-8")
   sealer_kver = check_output(shlex.split('find '+credentials_dir+' -name "sealer.kver"')).strip().decode("utf-8")
   ...
   backchannel_p12 = check_output(shlex.split('find '+credentials_dir+' -name "idp-backchannel.p12"')).strip().decode("utf-8")
   backchannel_crt = check_output(shlex.split('find '+credentials_dir+' -name "idp-backchannel.crt"')).strip().decode("utf-8")
   ...
   signing_crt = check_output(shlex.split('find '+credentials_dir+' -name "idp-signing.crt"')).strip().decode("utf-8")
   signing_key = check_output(shlex.split('find '+credentials_dir+' -name "idp-signing.key"')).strip().decode("utf-8")
   ...
   encryption_crt = check_output(shlex.split('find '+credentials_dir+' -name "idp-encryption.crt"')).strip().decode("utf-8")
   encryption_key = check_output(shlex.split('find '+credentials_dir+' -name "idp-encryption.key"')).strip().decode("utf-8")
   ...
   if (os.path.isfile(idp_credentials_pw)):
      ...
      if "ANSIBLE_VAULT" in idp_cred_pw:
         idp_cred_pw = check_output(shlex.split('ansible-vault view --vault-password-file '+ ans_vault_file+' '+ credentials_dir+'/'+idp_fqdn+'_pw.txt')).strip().decode("utf-8")
         return idp_cred_pw
      return idp_cred_pw
   else:
   ...
   for contactType, value in dict_yaml['contacts'].items():
    ...
    if (contactType == 'technical'):
     for contact in value:
       list_technical.append( { contact['name'] : contact['email'] } )
...
def get_idp_values(dict_yaml, ans_vault_file, dest='tmp', debug=False):
   ...
   idp_sealer_pw = get_sealer_keystore_pw(dict_yaml, ans_vault_file, dest, debug)
